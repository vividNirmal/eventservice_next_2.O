...
export function SortableElement({
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn("flex flex-wrap items-center gap-2 group relative bg-white border-1 rounded-lg p-2 mb-3 group transition-all duration-200 hover:bg-gray-100", isSelected ? 'border-gray-600 shadow-lg ring-1 ring-blue-200' : 'border-gray-200 hover:border-gray-300', isDragging ? 'opacity-50' : '')} onClick={handleClick}>
      {/* Drag Handle */}
      <div {...attributes} {...listeners} className="rounded-lg p-1 z-[1] absolute left-0 top-0 size-full shadow transition-opacity cursor-grab active:cursor-grabbing">
      ...
      {isSelected && (
        ...
        {element.fieldPermission === "DEFAULT_FIELD_NON_REMOVED" ? (          
          <Tooltip>
            <TooltipTrigger className="relative z-10 size-8 flex justify-center items-center">
              <OctagonAlert className="size-4 text-gray-300 cursor-help" />
            </TooltipTrigger>
            <TooltipContent className={"bg-black [&>span>svg]:bg-black [&>span>svg]:fill-black"}>
              <p>This is a default field and cannot be removed.</p>
            </TooltipContent>
          </Tooltip>
        ) : (
          <Button
            ...
            className="z-10 size-8 p-0 bg-white hover:bg-red-50 hover:border-red-200"
          >
