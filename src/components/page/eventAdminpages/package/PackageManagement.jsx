...
export default function PackageManagement() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await getRequest(
        `get-package?page=${currentPage}&pageSize=${selectedLimit}&searchQuery=${encodeURIComponent(
          searchTerm
        )}`
      );
      if (res.status === 1) {
        setPackage(res.data?.eventPackages || []);
        setTotalUsers(res.data.pagination.totalData);
  ...
  const handleSelectAll = (checked) => {
    if (checked) {
      setSelectedUsers(new Set(packageList.map((user) => user._id)));
    } else {
  ...
  try {
    const result = await postRequest("detele-package", {
      package_ids: deleteDialog.users // Send array directly
    });
                  ...
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-[50px]">
                        <Checkbox
                          checked={
                            selectedUsers.size === packageList.length &&
