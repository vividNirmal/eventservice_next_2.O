...
import { useCompanyImages } from "@/contexts/CompanyImageContext";
...
export default function HeaderEventuser() {
  ...
  // USE COMPANY IMAGES CONTEXT
  const { companyImages, getBannerForUserType, logo, fetchCompanyImages } = useCompanyImages();
  
  const pathname = usePathname();
  const router = useRouter();

  // Get current banner based on user type
  const currentBanner = useMemo(() => {
    if (!selectedUserType) return null;
    return getBannerForUserType(selectedUserType.typeName);
  }, [selectedUserType, getBannerForUserType]);

  // Fetch company images when user type changes or component mounts
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Dynamic Banner based on user type */}
      {currentBanner && (
