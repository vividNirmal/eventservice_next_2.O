      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-4 xl:gap-6">
          {showsData.map((item, index) => (
            <Link href={`/show-event/${item._id}`} key={index} className="max-w-1/2 w-1/3 grow sm:max-w-1/3 sm:w-1/4 lg:max-w-1/4 lg:w-1/6 group relative bg-background p-4 lg:p-6 lg:py-8 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 rounded-lg border border-border/50 overflow-hidden cursor-pointer">
              ...
              <div className="relative z-10 flex flex-col items-center text-center h-full">
                <div className={"mb-4 p-4 rounded-full bg-muted group-hover:bg-blue-500/10 transition-colors duration-300"}>
                  <Clapperboard />
                </div>
