...
const SmsTemplateList = () => {
  ...
  const handleCopyDetails = async (template) => {
    try {
      ...
      if (template.type === "sms" && template.subject) {
  ...
  const openCreateModal = () => {
    if (templateTypes.length === 0) {
      toast.error("Please create sms template types first");
      return;
    }
    setTemplateToEdit(null);
    setIsFormOpen(true);
  ...
  return (
    <>
      ...
      <TemplateFormSheet
        ...
        title={templateToEdit ? "Edit Sms Template" : "Create Sms Template"}
        description={
          templateToEdit
            ? "Update your sms template details."
            : "Create a new sms template for your communications."
        }
        submitButtonText={
