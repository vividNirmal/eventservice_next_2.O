...
const SmsTypeList = () => {
  ...
  const fetchTemplateTypes = async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        type: "sms", // Only fetch sms template types
  ...
  const handleAddTemplateType = async (data) => {
    ...
    try {
      ...
      if (response.status === 1) {
        toast.success("Sms type created successfully");
  ...
  const handleEditTemplateType = async (data) => {
    ...
    try {
      ...
      if (response.status === 1) {
        toast.success("Sms type updated successfully");
  ...
  const handleDeleteTemplateType = async () => {
    ...
    try {
      ...
      if (response.status === 1) {
        toast.success("Sms type deleted successfully");
      ...
      toast.error("Failed to delete sms type");
    }
  };
  ...
  return (
    <>
      ...
      <DeleteConfirmationDialog
        ...
        description={`Are you sure you want to delete this sms type? This action cannot be undone.`}
      />
    </>
  );
};

export default SmsTypeList;
