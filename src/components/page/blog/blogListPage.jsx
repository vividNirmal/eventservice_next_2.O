...
function BlogListPage() {
  ...
  const fetchBlogs = async () => {
    ...
    try {
      const res = await getRequest(
        `blogs-listing?page=${currentPage}&pageSize=${selectedLimit}&searchQuery=${encodeURIComponent(
          searchTerm
        )}`
      );
      if (res.status === 1) {
        setBlogList(res.data?.users || []);
  ...
  const handleSelectAll = (checked) => {
    if (checked) {
      setSelectedUsers(new Set(blogList.map((user) => user._id)));
    } else {
  ...
  const handleAddBlog = () => {
    router.push("/dashboard/blog-list/add-blog");
  };

  const handleEditBlog = (id) => {
    router.push(`/dashboard/blog-list/add-blog/${id}`);
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      formData.append("blog_ids[]", deleteDialog.users);
      const result = await postRequest("delete-event-blog", formData);
      if (result.status == 1) {
                  ...
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-[50px]">
                        <Checkbox
                          checked={
                            selectedUsers.size === blogList.length &&
                            blogList.length > 0
                          }
                    ...
                    {blogList.length > 0 ? (
                      blogList.map((blog, index) => (
                        <TableRow key={blog._id}>
                          <TableCell>
                            <Checkbox
                              checked={selectedUsers.has(blog._id)}
                              onCheckedChange={() => handleSelectUser(blog._id)}
                            />
                          </TableCell>
      ...
      <DeleteConfirmationDialog
        ...
        description={
          deleteDialog.users.length > 1
            ? `Are you sure you want to delete ${deleteDialog.users.length} blogs? This action cannot be undone.`
