...
const FaceScannerDetails = ({ faceData, onRedirect }) => {
  ...
  const isParticipantBlocked = () => {
    ...
    const participantData = faceData?.[2];
  ...
  const getUserName = () => {
    ...
    const userData = faceData?.[2]?.dynamic_fields;
    if (userData) {
  ...
  return (
    <div 
      ...
      className={`flex flex-col w-full max-w-md mx-auto bg-white rounded-2xl overflow-hidden shadow-xl transition-transform duration-200 ${isErrorResponse ? 'cursor-pointer hover:scale-105 active:scale-95' : ''}`}
    >
        ...
        {isErrorResponse ? (
          ...
          <>
            ...
            <Image
              ...
              className={`w-full h-full object-cover transition-opacity duration-300 ${imageLoading ? 'opacity-0' : 'opacity-100'} ${isParticipantBlocked() ? 'grayscale' : ''}`}
              alt="User Profile"
...
export default FaceScannerDetails;