...
import { buildCleanDeviceUrl, copyToClipboard, validateEventForUrl } from "@/utils/urlBuilder";
...
function ScanningDevicesPage() {
  ...
  useEffect(() => {
    const storedCompanyId =
      ...
      localStorage.getItem("loginuser.company_id");
    if (storedCompanyId) {
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-500">
            Please select an event to view scanning devices.
          </p>
        </div>
      </div>
    );
  ...
  async function handleCopyTicketUrl(scannerType) {
    try {    
        
      const formData = new FormData();
      formData.append("id", eventId);
      formData.append("type", scannerType); // Check-in type      
      ...
      if (config.status == 1) {
        ...
        if (copySuccess) {
          toast.success(
            "Clean device URL copied to clipboard! Share: " + deviceUrl
          );
        } else {
        ...
        toast.error(
          `Failed to generate device URL: ${config.message || "Unknown error"}`
        );
      }
    } catch (err) {
      console.error("Failed to generate/copy clean device URL:", err);
      toast.error(
        `Failed to copy device URL: ${err.message || "Unknown error"}`
      );
    }
  }
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Device URL</TableHead>
                    ...
                    <TableRow key={scanner._id}>
                      ...
                      <TableCell>
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleCopyTicketUrl(scanner.device_type)}
                      ...
                      <TableCell>
                        {getStatusBadge(scanner.expired_date)}
                      </TableCell>
                      <TableCell>
