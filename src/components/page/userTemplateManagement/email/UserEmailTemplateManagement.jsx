...
const UserEmailTemplateList = dynamic(() => import('./UserEmailTemplateList'), {
  ssr: false,
  loading: () => <div className="flex justify-center items-center h-64">Loading templates...</div>
});

const DefaultTemplateList = dynamic(() => import('../common/DefaultTemplateList'), {
  ssr: false,
  loading: () => <div className="flex justify-center items-center h-64">Loading default templates...</div>
});
...
const UserEmailTemplateManagement = ({eventId}) => {
  ...
  const activeTabContent = useMemo(() => {
    switch (activeTab) {
      case 'user-templates':
        return <UserEmailTemplateList eventId={eventId} />;
