...
export const PaperBadgeSettingFormSheet = ({
  isOpen,
  onClose,
  onSubmit,
  isSubmitting,
  ...
  const fetchTicketsWithUsage = async () => {
    ...
    try {
      ...
      const settingsResponse = await getRequest(
        `get-paper-badge-settings?eventId=${eventId}&limit=1000`
      );
      ...
      <SheetContent className="sm:max-w-xl flex flex-col overflow-hidden">
        ...
        <form onSubmit={formik.handleSubmit} className="flex flex-col h-full">
          <div className="p-6 pt-0 flex flex-col gap-4 grow overflow-y-auto">
            ...
            <p className="text-zinc-600 text-xs rounded-lg p-2 bg-zinc-100 border border-solid border-zinc-200">"Tickets that are already linked to other paper badge settings are disabled and cannot be selected."</p>
          </div>
